...
class GeneratePassScreen extends StatefulWidget {
  final VisitorPass? passToEdit;

  const GeneratePassScreen({Key? key, this.passToEdit}) : super(key: key);
...
class _GeneratePassScreenState extends State<GeneratePassScreen> with FormValidationMixin {
  ...
  void initState() {
    super.initState();
    _visitorNameController = TextEditingController(text: widget.passToEdit?.visitorName ?? '');
    _contactNumberController = TextEditingController(text: widget.passToEdit?.contactNumber ?? '');
    _purposeController = TextEditingController(text: widget.passToEdit?.purpose ?? '');
    _selectedDate = widget.passToEdit?.visitDate ?? DateTime.now();
  }
  ...
  Future<void> _handleSubmit() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (widget.passToEdit != null) {
          // Update existing pass
          await provider.updatePass(
            widget.passToEdit!.id,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.passToEdit != null ? 'Edit Visitor Pass' : 'Generate Visitor Pass'),
      ),
      body: Padding(
          ...
          child: ListView(
            children: [
              ...
              ElevatedButton(
                ...
                child: Text(widget.passToEdit != null ? 'Update Pass' : 'Generate Pass'),
              ),
                      ...
                      onPressed: () {
                        showDialog(
                          ...
                          builder: (context) => AlertDialog(
                            ...
                            content: const Text('Do you want to edit this visitor pass?'),
                            actions: [
                              TextButton(
